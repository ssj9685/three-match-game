...
export default class Grid {
  ...
  private getNextColor(
    ...
    if (
      number <= 16 &&
      (!forInit ||
        (this.forInitGridStat.x[x].blue < 2 &&
          this.forInitGridStat.y[y].blue < 2))
    ) {
      ...
      number <= 32 &&
      (!forInit ||
        (this.forInitGridStat.x[x].red < 2 &&
          this.forInitGridStat.y[y].red < 2))
    ) {
      ...
      number <= 48 &&
      (!forInit ||
        (this.forInitGridStat.x[x].green < 2 &&
          this.forInitGridStat.y[y].green < 2))
    ) {
      ...
      number <= 64 &&
      (!forInit ||
        (this.forInitGridStat.x[x].purple < 2 &&
          this.forInitGridStat.y[y].purple < 2))
    ) {
      ...
      number <= 80 &&
      (!forInit ||
        (this.forInitGridStat.x[x].amber < 2 &&
          this.forInitGridStat.y[y].amber < 2))
    ) {
      ...
      !forInit ||
      (this.forInitGridStat.x[x].grey < 2 &&
        this.forInitGridStat.y[y].grey < 2) ||
      count > 8
    ) {
  ...
  private setNearCanBeSelected(
    x: number,
    y: number,
    canBeselected: boolean
  ): void {
    const leftCell = this.get(x - 1, y);
    if (leftCell !== null) {
      leftCell.canBeSelected = canBeselected;
    }
    const rightCell = this.get(x + 1, y);
    if (rightCell !== null) {
      rightCell.canBeSelected = canBeselected;
    }
    const topCell = this.get(x, y + 1);
    if (topCell !== null) {
      topCell.canBeSelected = canBeselected;
    }
    const bottomCell = this.get(x, y - 1);
    if (bottomCell !== null) {
  ...
  select(x: number, y: number): boolean {
    if (this.canMove) {
      ...
      const alreadySelected: boolean = cell.selected;
      if (this.selectedCell === null || alreadySelected) {
        ...
        this.canMove = false;
        const selected = this.get(this.selectedCell.x, this.selectedCell.y);
        if (selected === null) {
          return false;
        }
        selected.selected = false;
        this.setNearCanBeSelected(
          this.selectedCell.x,
          this.selectedCell.y,
          false
        );
  ...
  getGridMatch(isCombo: boolean): MatchResult {
    const cellsToRemove: SimpleCell[] = [];
    const matches: Match[] = [];
    let currentColor: string = "";
    let currentSuite: number = 0;
    let elemInList: SimpleCell | undefined;
    for (let x: number = 0; x < this.squareSize; x++) {
      for (let y: number = 0; y < this.squareSize; y++) {
        const cell = this.get(x, y, false, this.cells);
        if (cell === null) {
          continue;
        }
        if (y === 0) {
        ...
        if (currentSuite === 2) {
          elemInList = cellsToRemove.find(
            (elem) => elem.x === x && elem.y === y - 2
          );
          if (elemInList === undefined) {
            cellsToRemove.push({ x, y: y - 2 });
          }
          elemInList = cellsToRemove.find(
            (elem) => elem.x === x && elem.y === y - 1
          );
          if (elemInList === undefined) {
    ...
    for (let y: number = 0; y < this.squareSize; y++) {
      for (let x: number = 0; x < this.squareSize; x++) {
        ...
        if (currentSuite === 2) {
          elemInList = cellsToRemove.find(
            (elem) => elem.x === x - 2 && elem.y === y
          );
          if (elemInList === undefined) {
            cellsToRemove.push({ x: x - 2, y });
          }
          elemInList = cellsToRemove.find(
            (elem) => elem.x === x - 1 && elem.y === y
          );
          if (elemInList === undefined) {
  ...
  fillGrid(matches: SimpleCell[]): Cell[] {
    ...
    for (let x: number = 0; x < this.squareSize; x++) {
      ...
      yMatches.forEach(() => {
        const newCell = this.getNextColor(x, newY, false);
  ...
  remove(x: number, y: number) {
    ...
    if (cell !== null) {
      ...
      for (let i: number = y; i < this.squareSize - 1; i++) {
        const editedCell = this.get(x, i + 1);
        if (editedCell !== null) {
          editedCell.y = i;
  ...
  invertCellsPosition(fx: number, fy: number, sx: number, sy: number) {
    const cellF = this.get(fx, fy);
    const cellS = this.get(sx, sy);
    if (cellF !== null) {
